...
export class FirebaseService {
  ...
  registerPart(part: Part){
    return this.firestore.collection(this.PATH).add({nome: part.nome, ano: part.ano, montadora: part.montadora, tipocarro: part.tipocarro, tracao: part.tracao, downloadURL: part.downloadURL, uid: part.uid});
  }
  updatePart(part: Part, id: string){
    return this.firestore.collection(this.PATH).doc(id).update({nome: part.nome, ano: part.ano, montadora: part.montadora, tipocarro: part.tipocarro, tracao: part.tracao, downloadURL: part.downloadURL, uid: part.uid});
  }
  ...
  uploadImage(imagem: any, part: Part){
    ...
    if(file.nome.split('/')[0] !== 'image'){
      console.error("Tipo não suportado!");
      return;
    }
    const path = `images/${part.ano}_${file.name}`;
